  ...
  const handleRemoveLecture = (chapterId, lectureIndex) => {
    setChapters(chapters.map(ch => {
      if (ch.chapterId === chapterId) {
        const newContent = [...ch.chapterContent];
        newContent.splice(lectureIndex, 1);
        return { ...ch, chapterContent: newContent };
      }
      return ch;
    }));
  };
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:pb-0 p-4 pt-8 pb-0'>
      <form>
        ...
        <div className='flex flex-col gap-1 mb-4'>
          ...
          <div ref={editorRef} className="bg-white h-40 border border-gray-300 rounded" />
        </div>
        ...
        <div className='flex items-center justify-between flex-wrap gap-4 mb-4'>
          ...
          <div className='flex items-center gap-3'>
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              ...
              {image && <img className='max-h-10 rounded shadow' src={URL.createObjectURL(image)} alt="thumb" />}
            </label>
          </div>
        </div>
        ...
        <div className='flex flex-col gap-1 mb-4'>
          ...
          <input
            ...
            min={0}
            max={100}
            className='outline-none py-2 px-3 rounded border border-gray-500 w-28'
            required
          />
        </div>
          ...
          {chapters.map((chapter, chapterIndex) => (
            <div key={chapter.chapterId} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                ...
                <span className='text-gray-500'>{chapter.chapterContent.length} Lecture(s)</span>
                <img src={assets.cross_icon} alt="remove" className='cursor-pointer' onClick={() => handleChapter('remove', chapter.chapterId)} />
              </div>
                  ...
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                      <span>
                        ...
                        <a href={lecture.lectureUrl} target='_blank' className='text-blue-500 ml-1'>Link</a> - {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}
                      </span>
                      <img src={assets.cross_icon} alt="remove" className='cursor-pointer' onClick={() => handleRemoveLecture(chapter.chapterId, lectureIndex)} />
                    </div>
                  ))}
